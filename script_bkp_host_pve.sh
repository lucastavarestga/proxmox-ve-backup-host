...
/usr/sbin/qm list | grep running > "$PASTA_DIA/qm_list_running-${DATA_HOJE}_${HORA_AGORA}.txt" || true
/usr/sbin/qm list | grep stopped > "$PASTA_DIA/qm_list_stopped-${DATA_HOJE}_${HORA_AGORA}.txt" || true
...
FILES_TO_BACKUP=(
    ...
    "$PASTA_DIA/pveversion.txt"
    "$PASTA_DIA/qm_list_running-${DATA_HOJE}_${HORA_AGORA}.txt"
    "$PASTA_DIA/qm_list_stopped-${DATA_HOJE}_${HORA_AGORA}.txt"
)
...
if [ ${#EXISTING_FILES[@]} -eq 0 ]; then
    echo "[$(date)] Nenhum arquivo encontrado para backup. Abortando."
    exit 1
fi
...
# --- LIMPEZA ---
find "$DIR_BK" -type f -mtime +$DIAS_RETENCAO -delete
find "$DIR_BK" -type d -empty -delete
